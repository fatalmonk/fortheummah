...
EXTERNAL_ICON = '<svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 17 17 7M7 7h10v10"/></svg>'

def polish_document(html):
    return html.replace("WhatsApp enquiry ↗", "WhatsApp " + EXTERNAL_ICON).replace("WhatsApp ↗", "WhatsApp " + EXTERNAL_ICON)
...
policy_page = policy_page.replace('class="product-page"', 'class="product-page policy-page"')
policy_sections = [
    ("prices", "Prices and products"),
    ("delivery", "Delivery"),
    ("payment", "Payment and order confirmation"),
    ("cancellation", "Cancellation and delivery inspection"),
    ("returns", "Returns, defects, damage and missing pieces"),
    ("contact", "Contact"),
]
policy_toc = '<nav class="policy-toc" aria-label="On this page">' + ''.join(
    f'<a href="#{key}">{title}</a>' for key, title in policy_sections
) + '</nav>'
for index, (key, title) in enumerate(policy_sections):
    start = policy_toc if index == 0 else '</section>'
    policy_page = policy_page.replace(f'<h2>{title}</h2>', f'{start}<section id="{key}" aria-labelledby="{key}-title"><h2 id="{key}-title">{title}</h2>')
policy_page = policy_page.replace('</main>', '</section></main>')
policy_page = policy_page.replace('<a href="/">← Back to the catalogue</a>', '<a class="back-link" href="/">← Back to the catalogue</a>')
(SITE / "policies.html").write_text(polish_document(policy_page))
...
</head><body><a class="skip-link" href="#main">Skip to product details</a><header class="site-header"><a class="brand" href="/">For the Ummah<span>MEANING IN EVERY PIECE</span></a><nav aria-label="Main navigation"><a href="/">Home</a><a href="/#collection">Collection</a><a href="/policies.html">Policies</a><a class="nav-contact" href="https://wa.me/8801731944544" target="_blank" rel="noopener noreferrer">WhatsApp enquiry ↗</a></nav></header><main id="main" class="product-page"><p class="eyebrow"><a href="/">THE COLLECTION</a> / {category} / {pid}</p><h1>{title}</h1><p class="product-subtitle">Puzzle design · Bangladesh · Enquiries only; orders are not yet being accepted.</p><figure class="product-art"><a href="{full}" target="_blank" rel="noopener noreferrer" aria-label="Open full-resolution design sheet in a new tab"><img src="{full}" width="1774" height="887" alt="{title} three-panel design sheet with puzzle image, proposed packaging, and a frame style reference" fetchpriority="high"></a><figcaption>Open full-resolution design sheet ↗</figcaption></figure><p class="content-note">Age guidance is 14+; final labeling will be confirmed with supplier/sample approval. The frame shown is a style reference and is not included.</p>{extra}<section class="product-enquire" aria-labelledby="sizes-title">
<h2 id="sizes-title">Choose your puzzle size</h2>
<p class="fine">Proposed specifications (supplier/sample confirmation pending).</p>
<div class="variant-grid">
<div class="variant-card"><span>500 pieces</span><strong>৳3,000</strong><p>52 × 38 cm landscape<br>38 × 52 cm portrait</p></div>
<div class="variant-card"><span>1,000 pieces</span><strong>৳4,000</strong><p>70 × 50 cm landscape<br>50 × 70 cm portrait</p></div>
</div>
<dl class="spec-list">
<div><dt>Material &amp; finish</dt><dd>Cardboard with glossy finish</dd></div>
<div><dt>Packaging &amp; age</dt><dd>Printed puzzle box · age suitability 14+</dd></div>
<div><dt>Optional wooden frame kit</dt><dd>+৳3,000 · a frame is not included with the puzzle</dd></div>
<div><dt>Delivery</dt><dd>Charged separately. <a href="/policies.html#delivery">View delivery details</a></dd></div>
</dl>
<p class="fine">Supplier specification, physical sample and production lead time remain pending. Designs with Arabic or religious material require qualified review before manufacture.</p><a class="button" href="/?product={pid}#collection">Enquire about {pid} <svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M5 12h14m-6-6 6 6-6 6"/></svg></a><p class="fine">This opens the selected-design enquiry form. It does not place or confirm an order.</p></section><p><a class="back-link" href="/#collection">← Back to all designs</a></p></main><footer><a class="brand" href="/">For the Ummah<span>MEANING IN EVERY PIECE</span></a><p><a href="/policies.html">Delivery &amp; customer policies</a><br><a href="https://wa.me/8801731944544">WhatsApp: 01731944544</a></p><p class="fine">Supplier/sample approval is pending. Arabic lettering, religious text and references require qualified review before manufacture.</p></footer></body></html>\n'''

    out_dir = SITE / "product" / pid
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "index.html").write_text(polish_document(html))
